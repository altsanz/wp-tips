...
function avia_include_shortcode_template($paths)
{
	...
	array_unshift($paths, $template_url.'/avia-plugins/');

	return $paths;
}
```

And inside avia-plugins folder add the templates that override and create new element in ALB:
...
if ( !class_exists( 'avia_sc_masonry_gallery_more_columns' ) ) 
{
	class avia_sc_masonry_gallery_more_columns extends aviaShortcodeTemplate
	{	
			/**
			...
			{
				$this->config['name']			= __('Masonry Gallery +10 col', 'avia_framework' );
				$this->config['tab']			= __('Media Elements', 'avia_framework' );
				$this->config['icon']			= AviaBuilder::$path['imagesURL']."sc-masonry-gallery.png";
				$this->config['order']			= 5;
				$this->config['target']			= 'avia-target-insert';
				$this->config['shortcode'] 		= 'av_masonry_gallery_10col';
				$this->config['tooltip'] 	    = __('Display a fullwidth masonry/grid gallery, up to 10 columns', 'avia_framework' );
			...
			function popup_elements()
			{
				...
				array(
					...
					"subtype" => array(
						...
						__('12 Columns',  'avia_framework' ) =>'12',
